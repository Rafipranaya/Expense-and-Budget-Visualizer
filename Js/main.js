...
function initTheme() {
  const savedTheme = localStorage.getItem("ebv_theme") || "dark";
...
function _doRenderChart() {
  ...
  if (chartInstance) {
    ...
    chartInstance.options.plugins.tooltip.callbacks.label = (c) => `  ${c.label}: ${formatCurrency(c.parsed)}`;
